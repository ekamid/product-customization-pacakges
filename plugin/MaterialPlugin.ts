...
class MaterialPlugin {
  ...
  getMaterialTypeList() {
    return axios.get(`/data/material-types.json`).then((res) => {
  ...
  getTemplMaterialTypeList() {
    return axios.get(`/data/templ-types.json`).then((res) => {
      const list = res.data.data.map((item: any) => {
        return {
          value: item.id,
          label: item.attributes.name,
          list: item.attributes.templs.data.map((info: any) => {
            console.log(this._getMaterialInfoUrl(info));
            return {
              value: info.id,
              label: info.attributes.name,
              src: this._getMaterialInfoUrl(info),
